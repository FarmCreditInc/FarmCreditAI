...
def retrieve_similar_chunks(query, top_k=3, bedrock=bedrock, embedding_model_id=embedding_model_id, index=index):
    """
...
def generate_answer(query,conversation_trail,user_info,retrieved_chunks,bedrock=bedrock, generation_model_id=generation_model_id):
    """
    ...
    try:
        ...
        prompt = f"""
        You are a helpful chatbot for FarmCredit, a platform supporting Nigerian youth farmers. You job is to engage in conversation with the user, You provide the following services:
        1. You are a helpful assistant that answers questions about FarmCredit's services using the context provided.
        2. You can provide details about the customer's wallet balance, available loans, and credit score using the user_info provided.
        
        Use the conversation trail to understand the context of the conversation. The conversation trail is a list of previous messages exchanged between the user and the assistant.
...
Query: The Query is the user's question or request. It is the last message in the conversation trail.
{query}
...
def converse_pipeline(query, user_info, conversation_trail):
    """
    ...
    try:
        ...
        retrieved_chunks = retrieve_similar_chunks(query, top_k=3)
        # Generate answer using retrieved chunks
        answer = generate_answer(query,conversation_trail,user_info,retrieved_chunks)
        return answer
    except Exception as e:
        print(f"Error in CONVERSE pipeline: {e}")
